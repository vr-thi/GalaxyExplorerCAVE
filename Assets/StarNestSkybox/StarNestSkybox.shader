	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_t v) {
				...
				float3 cameraPos = float3(0,1,0); 	// The camera's current position
			...
			half4 frag (v2f IN) : SV_Target {
				...
				//scroll over time
				from += _Scroll.xyz * _Scroll.w * 0;
				//scroll from camera position
				from += _WorldSpaceCameraPos * _CamScroll * 0;
				
				
				//Apply rotation if enabled
				float3 rot = _Rotation.xyz * _Rotation.w * time *0;
				if (length(rot) > 0) {
